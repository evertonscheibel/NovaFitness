# Script Simplificado de Instala??o do MongoDB
# Instala??o direta via download

Write-Host ""
Write-Host "?? MongoDB Local - Instala??o Simplificada" -ForegroundColor Cyan
Write-Host "=" * 50 -ForegroundColor Cyan
Write-Host ""

# Verificar se MongoDB j? est? instalado
Write-Host "?? Verificando instala??es existentes..." -ForegroundColor Cyan
...
if (-not $mongoInstalled) {
    Write-Host "? MongoDB n?o encontrado." -ForegroundColor Red
    Write-Host ""
    ...
    Write-Host "   5. IMPORTANTE: Marque a op??o 'Install MongoDB as a Service'" -ForegroundColor Yellow
    Write-Host "   6. Execute este script novamente ap?s a instala??o" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "??  Script pausado. Execute novamente ap?s instalar o MongoDB." -ForegroundColor Yellow
    exit 0
}

# MongoDB est? instalado, continuar com configura??o
Write-Host ""
Write-Host "??  Configurando projeto NovaFitness..." -ForegroundColor Cyan
Write-Host ""
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "??  Aviso na sincroniza??o (pode ser normal)" -ForegroundColor Yellow
}
